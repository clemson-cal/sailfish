...
vt12_2s = np.sqrt((p1 - p2) * (ehat - e2) / ((ehat + p2) * (e2 + p1)))
...
vt12_2r = -1.0 * (s1 - s2) / (s1 + s2)

print(v12, vt12_2s, vt12_sr, vt12_2r)
...
elif v12 > vt12_sr and v12 <= vt12_2s:
    pmin = min(p1, p2)
    pmax = max(p1, p2)
...
elif v12 > vt12_2r and v12 <= vt12_sr:
    pmin = 0.0
    pmax = min(p1, p2)
    result = optimize.brentq(v12_2s, pmin, pmax, full_output=True)
    p3 = result[0]
