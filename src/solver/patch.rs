...
pub mod host {
    ...
    impl Patch {
        ...
        pub fn contains(&self, other: &Self) -> bool {
            unsafe { ffi::patch_contains(self.0, other.0) != 0 }
        }

        #[cfg(feature = "cuda")]
        pub fn to_device(&self) -> device::Patch {
            unsafe { device::Patch(ffi::patch_clone_to_device(self.0)) }
        }
    ...
    impl Clone for Patch {
        fn clone(&self) -> Self {
            Self(unsafe { ffi::patch_clone(self.0) })
        }
    }

    impl Drop for Patch {
        fn drop(&mut self) {
            unsafe { ffi::patch_del(self.0) }
        }
    }
}
...
pub mod device {
    ...
    impl Patch {
        ...
        pub fn to_host(&self) -> host::Patch {
            unsafe { host::Patch(ffi::patch_clone_to_host(self.0)) }
        }
    }
